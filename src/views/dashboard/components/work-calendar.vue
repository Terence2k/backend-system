<template>
  <div>
    <el-row type="flex" justify="end">
      <el-select v-model="y" @change="changeData">
        {{ y }}
        <el-option v-for="item in 12" :key="item" :value="item" :label="item + '月'">{{ item + '月' }}</el-option>
      </el-select>

      <el-select v-model="cy" @change="changeData">
        <el-option v-for="item in yearl" :key="item" :label="item + '年'" :value="item ">{{ item + '年' }}</el-option>
      </el-select>
...
export default {
  ...
  methods: {
    ...
    changeData() {
      console.log(this.cy, 'nian')
      console.log(this.y, '55')
      this.cd = new Date(`${this.cy}-${this.y}-1`)
    }
  }
}
</script>
